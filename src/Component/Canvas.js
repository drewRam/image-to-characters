...
class AsciiEffect {
    ...
    // Depending on color of RGB, we will get a character to represent a pixel cell
        ...
        for(let y = 0; y < this.#pixels.height; y += cellSize) {
            for(let x = 0; x < this.#pixels.width; x += cellSize) {
                ...
                if (this.#pixels.data[pos + 3] > 128){
                    ...
                    // Exclude dark and/or dim colors | Can manipulate colors here further before pushing into a new cell
                    /*
                    if (total > 200) 
    ...
    #drawAscii = () => {
        // Clear canvas from old paint with built in function
...
const Canvas = ( props ) => {
    const draw = ( ctx, canvas ) => {
        /**
         * imgage is converted from png file to base64 
         * https://onlinepngtools.com/convert-png-to-base64 
         * Must have valid data url prefix of 'data:image/png;base64,' for image.src to perform correctly
         */ 
        ...
        image.onload = function initialize() {
            ...
            /**
             * Each cell will be param * param pixel cells big
             * Each cell has color, symbol, x and y properties
             * Param should range between 1-10 for best qualities
             */
            effect.draw(2);
        }
    };
